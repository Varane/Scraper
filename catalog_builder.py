...
def ensure_files_exist() -> None:
    if not CATALOG_PATH.exists():
        try:
            ...
            log_error(f"Failed to initialize catalog.json: {exc!r}")
...
def existing_oems_for_part(catalog: Dict, brand: str, model: str, part: str) -> Set[str]:
    ...
    for entry in existing:
        main_oem = entry.get("oem_main")
        if main_oem:
            collected.add(str(main_oem).upper())
        for ref in entry.get("oem_cross_refs", []):
            collected.add(str(ref).upper())
    return collected
...
def request_with_retry(url: str, session: requests.Session, proxy: Optional[str] = None) -> Optional[str]:
    ...
    for attempt in range(5):
        ...
        try:
            ...
            log_error(f"Non-200 status {response.status_code} for url={url}")
        except requests.RequestException as exc:
            log_error(f"Request attempt {attempt + 1} failed for url={url}: {exc!r}")
    return None
...
def parse_currency(price_text: str) -> (Optional[str], Optional[float]):
...
def extract_listings(query: str, session: requests.Session, proxy: Optional[str], existing_oems: Iterable[str], brand: str, model: str, part: str) -> List[Dict[str, object]]:
        ...
        for model in models:
            ...
            for part in PARTS:
                ...
                for query in queries:
                    part_results.extend(
                        extract_listings(query, session, proxy, existing_oems.union({r.get("oem_main", "") for r in part_results}), brand, model, part)
                    )
                    sleep_with_jitter()
                if part_results:
                    catalog[brand][model][part] = part_results
                    save_catalog(catalog)
                    log_info(f"Saved {len(part_results)} entries for {brand} {model} {part}")
    save_catalog(catalog)
...
def main() -> None:
    ...
    try:
        build_catalog(catalog, proxy=args.proxy)
